...
export const METRIC_META: Record<MetricKey, { label: string; unit: string; good: number; bad: number; lowerIsBetter: boolean }> = {
  LCP: { label: "Largest Contentful Paint", unit: "ms", good: 2500, bad: 4000, lowerIsBetter: true },
  INP: { label: "Interaction to Next Paint", unit: "ms", good: 200, bad: 500, lowerIsBetter: true },
  CLS: { label: "Cumulative Layout Shift", unit: "", good: 0.1, bad: 0.25, lowerIsBetter: true },
  FCP: { label: "First Contentful Paint", unit: "ms", good: 1800, bad: 3000, lowerIsBetter: true },
  TBT: { label: "Total Blocking Time", unit: "ms", good: 200, bad: 600, lowerIsBetter: true },
  PERF: { label: "Performance Score", unit: "", good: 90, bad: 50, lowerIsBetter: false },
};
...
export const REPOS: Repo[] = [
  { id: "web", name: "web", fullName: "acme/web", defaultBranch: "main", health: 92, openPrs: 4, failingPrs: 1, lastRun: "2m ago", budgets: defaultBudgets },
  { id: "storefront", name: "storefront", fullName: "acme/storefront", defaultBranch: "main", health: 78, openPrs: 7, failingPrs: 2, lastRun: "12m ago", budgets: defaultBudgets },
  { id: "docs", name: "docs", fullName: "acme/docs", defaultBranch: "main", health: 98, openPrs: 1, failingPrs: 0, lastRun: "1h ago", budgets: defaultBudgets },
  { id: "admin", name: "admin", fullName: "acme/admin", defaultBranch: "main", health: 64, openPrs: 3, failingPrs: 3, lastRun: "5m ago", budgets: defaultBudgets },
];
...
function pickStatus(metrics: Record<MetricKey, number>, budgets: Budget[]): PrStatus {
  ...
  for (const repo of REPOS) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const metrics: Record<MetricKey, number> = {
        ...
        PERF: Math.min(100, Math.max(20, baseline.PERF - Math.floor(drift * 18))),
      };
      ...
      out.push({
        ...
        branch: `feat/${titles[(id - 1) % titles.length].toLowerCase().replace(/[^a-z0-9]+/g, "-").slice(0, 24)}`,
        status,
...
export function timeSeries(repoId: string, days = 30) {
  ...
  const out: { date: string; lcp: number; inp: number; cls: number; perf: number }[] = [];
  let lcp = 2100, inp = 160, cls = 0.07, perf = 92;
...
export const ALERTS: AlertItem[] = [
  { id: "a1", repoId: "admin", channel: "slack", level: "critical", title: "LCP budget exceeded", message: "PR #1010 raised LCP to 4.2s (+1.4s vs main)", createdAt: "5m ago" },
  { id: "a2", repoId: "storefront", channel: "discord", level: "warning", title: "CLS trending up", message: "3-day rolling average crossed 0.12", createdAt: "1h ago" },
  { id: "a3", repoId: "web", channel: "email", level: "info", title: "Weekly digest", message: "12 PRs analyzed, 1 regression caught", createdAt: "1d ago" },
  { id: "a4", repoId: "admin", channel: "slack", level: "critical", title: "Performance score dropped", message: "Score fell from 91 to 64 on PR #1011", createdAt: "2d ago" },
];
...
export const MEMBERS: Member[] = [
  ...
  { id: "m4", name: "Ngozi Okafor", email: "ngozi@acme.dev", role: "Developer" },
