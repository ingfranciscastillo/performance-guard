				...
				<table className="w-full text-sm">
					<thead className="bg-muted/50 text-xs uppercase tracking-wider text-muted-foreground">
						<tr>
							...
							<th className="text-left font-medium px-5 py-3">Open PRs</th>
					...
					<tbody className="divide-y divide-border">
						{REPOS.map((r) => (
							<tr key={r.id} className="hover:bg-muted/30">
								...
								<td className="px-5 py-3 font-mono">{r.openPrs}</td>
								<td className="px-5 py-3">
									<span
										className={`font-mono ${r.health > 85 ? "text-success" : r.health > 70 ? "text-warning-foreground" : "text-destructive"}`}
									>
										{r.health}/100
									</span>
								</td>
